...
from codebase.models.code import CodeElementKind, CodeElement, MethodElement, ParameterElement, \
    FieldElement
...
#            try:
#                hierarchy = self.hierarchies.get(True,2)
#            except:
#                break
            child_fqn = hierarchy[0]
            child = CodeElement.objects.filter(codebase=self.codebase).get(fqn=child_fqn)
            for parent_fqn in hierarchy[1:]:
                try:
                    parent_element = CodeElement.objects.filter(codebase=self.codebase).get(fqn=parent_fqn)
                    child.parents.add(parent_element)
                except:
                    pass
            # No need to save manytomany it seems
#            child.save()
            self.progress_monitor.work('Parsed %s hierarchy: %i parents' % (child_fqn,len(hierarchy)-1))
            self.hierarchies.task_done()

class CUWorker(Thread):
    '''Worker that processes a compilation unit'''

    def __init__(self, queue, codebase, hierarchies, gateway, progress_monitor):
        '''
    ...
    def run(self):
        while True:
            (cu, package_code_element, cu_name, work_amount) = self.queue.get()
            self.progress_monitor.info('Parsing {0}'.format(cu_name))
            try:
                for type in cu.types():
                    self._parse_type(type, package_code_element)
            except Exception:
                print_exc()
            ...
            self.progress_monitor.work('Parsed {0}'.format(cu_name), work_amount)

    def _parse_type(self, type, container_code_element):
        type_binding = type.resolveBinding()
        java_element = type_binding.getJavaElement()
        (simple_name, fqn) = clean_java_name(type_binding.getQualifiedName())

        type_code_element = CodeElement(codebase=self.codebase, simple_name=simple_name, fqn=fqn, eclipse_handle=java_element.getHandleIdentifier(), parser=JAVA_PARSER)
        ...
        self._parse_type_members(type, type_code_element)
 
        self._parse_type_hierarchy(type, type_binding, type_code_element)

    def _parse_type_members(self, type, type_code_element):
        for body_declaration in type.bodyDeclarations():
            node_type = body_declaration.getNodeType()
            if node_type == self.annotation_type or node_type == self.enumeration_type or node_type == self.type_type:
                ...
                self._parse_annotation_field(body_declaration, type_code_element) 

        # Special case for enumerations...        
        if type_code_element.kind == self.enumeration_kind:
            for body_declaration in type.enumConstants():
                self._parse_enumeration_value(body_declaration, type_code_element)

        # Find method overloads
                
    def _parse_type_hierarchy(self, type, type_binding, type_code_element):
        supertypes = [type_code_element.fqn]
        super_class = type_binding.getSuperclass()
        if super_class != None:
            (_, fqn) = clean_java_name(super_class.getQualifiedName())
            if not fqn.startswith('java.'):
    ...
    def _parse_method(self, method, container_code_element):
        ...
        if not self._is_private(method_binding):
            ...
            (_, fqn) = clean_java_name(method_binding.getDeclaringClass().getQualifiedName())
            fqn = fqn + '.' + simple_name
            parameters = method_binding.getParameterTypes()
            parameter_declarations = method.parameters()
            params_length = len(parameters)
            (return_simple_name, return_fqn) = clean_java_name(method_binding.getReturnType().getQualifiedName())
            method_code_element = MethodElement(codebase=self.codebase, kind = self.method_kind, simple_name=simple_name, fqn=fqn, parameters_length=params_length, eclipse_handle = java_element.getHandleIdentifier(), return_simple_name=return_simple_name, return_fqn = return_fqn, parser=JAVA_PARSER)
            ...
            for i, parameter in enumerate(parameters):
                (type_simple_name, type_fqn) = clean_java_name(parameter.getQualifiedName())
                simple_name= fqn = parameter_declarations[i].getName().getIdentifier()
                parameter_code_element = ParameterElement(codebase=self.codebase, kind = self.method_parameter_kind, simple_name=simple_name, fqn=fqn, type_simple_name=type_simple_name, type_fqn=type_fqn, index=i, attcontainer=method_code_element, parser=JAVA_PARSER)
                parameter_code_element.save()
        
            # TODO method hierarchy
            self._parse_method_hierarchy(method, method_binding, method_code_element)
    
    def _parse_method_hierarchy(self, method, method_binding, method_code_element):
        # Not for now...
        pass
    ...
    def _parse_field(self, field, container_code_element):
        for fragment in field.fragments():
            ...
            if not self._is_private(field_binding):
                ...
                (_, fqn) = clean_java_name(field_binding.getDeclaringClass().getQualifiedName())
                fqn = fqn + '.' + simple_name
                (type_simple_name, type_fqn) = clean_java_name(field_binding.getType().getQualifiedName())
                field_code_element = FieldElement(codebase=self.codebase, kind = self.field_kind, simple_name=simple_name, fqn=fqn, eclipse_handle=java_element.getHandleIdentifier(),type_simple_name=type_simple_name, type_fqn = type_fqn, parser=JAVA_PARSER)
    ...
    def _parse_enumeration_value(self, value, container_code_element):
        ...
        if not self._is_private(field_binding):
            ...
            fqn = fqn + '.' + simple_name
            (type_simple_name, type_fqn) = clean_java_name(field_binding.getType().getQualifiedName())
            field_code_element = FieldElement(codebase=self.codebase, kind = self.enumeration_value_kind, simple_name=simple_name, fqn=fqn, eclipse_handle=java_element.getHandleIdentifier(),type_simple_name=type_simple_name, type_fqn = type_fqn, parser=JAVA_PARSER)
            field_code_element.save()
            field_code_element.containers.add(container_code_element)
    ...
    def _parse_annotation_field(self, field, container_code_element):
        ...
        if not self._is_private(method_binding):
            ...
            (_, fqn) = clean_java_name(method_binding.getDeclaringClass().getQualifiedName())
            fqn = fqn + '.' + simple_name
            (type_simple_name, type_fqn) = clean_java_name(method_binding.getReturnType().getQualifiedName())
            field_code_element = FieldElement(codebase=self.codebase, kind = self.annotation_field_kind, simple_name=simple_name, fqn=fqn, eclipse_handle=java_element.getHandleIdentifier(),type_simple_name=type_simple_name, type_fqn = type_fqn, attcontainer=container_code_element, parser=JAVA_PARSER)
            field_code_element.save()
            field_code_element.containers.add(container_code_element)
...
class JavaParser(object):
    ...
    def __init__(self, codebase, project_key, opt_input):
        '''
        :param project_name: The name of the project in the Eclipse workspace.
